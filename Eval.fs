...
let rec eval_expr (env : value env) (e : expr) : value =
    ...
    | BinOp (e1, "<", e2) -> binop_bool (<) (<) env e1 e2
    | BinOp (e1, "=", e2) -> binop_op_equal (=) env e1 e2
    | BinOp (e1, ">=", e2) -> binop_bool (>=) (>=) env e1 e2
    | BinOp (e1, "<=", e2) -> binop_bool (<=) (<=) env e1 e2
    | BinOp (e1, "<>", e2) -> binop_bool (<>) (<>) env e1 e2
...
and binop_op_equal op env e1 e2 = 
    let v1 = eval_expr env e1
    let v2 = eval_expr env e2
    match v1,v2 with
    | VLit (LInt x), VLit (LInt y) -> VLit(LBool (op x y))
    | _ -> unexpected_error "eval_expr: the equal operator works only with pairs of int"
...
and bool_op op env e1 e2 = 
    ...
    | _ -> unexpected_error "eval_expr: the and operator works only on bool"